                                             ...
                                             rmm::mr::device_memory_resource* mr)
{
  ...
  auto token_counts   = cudf::make_numeric_column(cudf::data_type{type_to_id<size_type>()},
                                                strings_count,
                                                cudf::mask_state::UNALLOCATED,
                                                stream,
                                                mr);
  auto d_token_counts = token_counts->mutable_view().data<size_type>();
                                          ...
                                          rmm::mr::device_memory_resource* mr)
{
  ...
  rmm::device_uvector<size_type> token_offsets(strings_count + 1, stream);
  thrust::inclusive_scan(rmm::exec_policy(stream),
                         d_token_counts.template begin<size_type>(),
                         d_token_counts.template end<size_type>(),
                                                 ...
                                                 rmm::mr::device_memory_resource* mr)
{
  ...
  auto offset  = cudf::detail::get_value<size_type>(offsets, strings_column.offset(), stream);
  auto chars_bytes =
    cudf::detail::get_value<size_type>(offsets, strings_column.offset() + strings_count, stream) -
    offset;
  ...
  auto offsets_column = cudf::make_numeric_column(cudf::data_type{cudf::type_to_id<size_type>()},
                                                  num_characters + 1,
                                                  cudf::mask_state::UNALLOCATED,
                                                  stream,
                                                  mr);
  auto d_new_offsets  = offsets_column->mutable_view().begin<size_type>();
  thrust::copy_if(
    rmm::exec_policy(stream),
    thrust::make_counting_iterator<size_type>(0),
    thrust::make_counting_iterator<size_type>(chars_bytes + 1),
    d_new_offsets,
