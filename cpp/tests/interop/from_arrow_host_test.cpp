...
get_nanoarrow_host_tables(cudf::size_type length, cudf::memory_resources mr)
{
  ...
  auto keys = cudf::test::to_host<int64_t>(view.keys(), cudf::get_default_stream(), mr).first;
  auto indices =
    cudf::test::to_host<uint32_t>(view.indices(), cudf::get_default_stream(), mr).first;
