...
class GenericIndex(SingleColumnFrame, BaseIndex):
    ...
    @classmethod
    @_cudf_nvtx_annotate
    def _from_data(cls, data: MutableMapping, name: Any = no_default) -> GenericIndex:
    ...
    def get_loc(self, key, method=None, tolerance=None):
        ...
        key_as_table = cudf.core.frame.Frame({"None": as_column(key, length=1)})
    ...
    def to_pandas(self, *, nullable: bool = False) -> pd.Index:
        return pd.Index(self._values.to_pandas(nullable=nullable), name=self.name)
...
class DatetimeIndex(GenericIndex):
    ...
    def to_pandas(self, *, nullable: bool = False) -> pd.DatetimeIndex:
        ...
        if isinstance(self.dtype, pd.DatetimeTZDtype):
            nanos = self._values.astype(pd.DatetimeTZDtype("ns", self.dtype.tz))
        else:
...
def interval_range(
    ...
    interval_col = column.build_interval_column(left_col, right_col, closed=closed)
...
class IntervalIndex(GenericIndex):
    ...
    def __init__(
        ...
        if isinstance(data, IntervalColumn):
            data = data
        elif isinstance(data, pd.Series) and isinstance(data.dtype, pd.IntervalDtype):
            ...
            data = column.column_empty_like_same_mask(column.as_column(data), dtype)
        else:
            data = column.as_column(data)
    ...
    def __init__(self, values, copy=False, **kwargs):
        ...
        if isinstance(values, StringColumn):
            ...
            if not is_string_dtype(values.dtype):
                raise ValueError("Couldn't create StringIndex from passed in object")
...
class Index(BaseIndex, metaclass=IndexMeta):
    ...
    def __new__(
        ...
        **kwargs,
        ...
        assert cls is Index, "Index cannot be subclassed, extend BaseIndex instead."
        if tupleize_cols is not True:
            raise NotImplementedError("tupleize_cols != True is not yet supported")
